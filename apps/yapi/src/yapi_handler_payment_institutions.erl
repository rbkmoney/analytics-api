-module(yapi_handler_payment_institutions).
...
process_request('GetPaymentInstitutions', Req, #{woody_context := WoodyContext}) ->
    try
        Residence = yapi_handler_encoder:encode_residence(genlib_map:get(residence, Req)),
        Realm = genlib_map:get(realm, Req),
        {ok, PaymentInstObjects} = yapi_domain:get_payment_institutions(WoodyContext),
        Resp =
            lists:filtermap(
                fun(P) ->
...
process_request('GetPaymentInstitutionByRef', Req, #{woody_context := WoodyContext}) ->
    ...
    case yapi_domain:get({payment_institution, ?payment_institution_ref(PaymentInstitutionID)}, WoodyContext) of
...
encode_optional_currency(SymbolicCode) -> yapi_handler_encoder:encode_currency(SymbolicCode).
...
decode_payment_institution_obj(#domain_PaymentInstitutionObject{ref = Ref, data = Data}) ->
    genlib_map:compact(#{
        ...
        <<"residences">> =>
            [
                yapi_handler_decoder_party:decode_residence(R) ||
...
decode_payment_terms(#domain_PaymentsServiceTerms{currencies = Currencies, categories = Categories}) ->
    genlib_map:compact(#{
        <<"currencies">> => decode_payment_terms(fun yapi_handler_decoder_utils:decode_currency    /1, Currencies),
        <<"categories">> => decode_payment_terms(fun yapi_handler_decoder_utils:decode_category_ref/1, Categories)
    });
...
decode_business_schedules_selector({value, Val}) when is_list(Val) ->
    lists:map(fun yapi_handler_decoder_utils:decode_business_schedule_ref/1, Val);
