...
process_request('CreateReport', Req, Context) ->
    ...
    case anapi_handler_utils:service_call({reporting, 'GenerateReport', [ReportRequest, ReportType]}, Context) of
        {ok, ReportId} ->
...
process_request('DownloadFile', Req, Context) ->
    Call = {
        reporting,
        'GetReport',
        [anapi_handler_utils:get_party_id(Context), maps:get(shopID, Req), maps:get(reportID, Req)]
    },
    case anapi_handler_utils:service_call(Call, Context) of
...
get_default_url_lifetime() ->
    ...
    Lifetime = application:get_env(anapi, reporter_url_lifetime, ?DEFAULT_URL_LIFETIME),
