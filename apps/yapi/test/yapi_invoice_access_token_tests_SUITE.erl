-module(yapi_invoice_access_token_tests_SUITE).
...
init_per_group(operations_by_invoice_access_token_after_invoice_creation, Config) ->
    MockServiceSup = yapi_ct_helper:start_mocked_service_sup(?MODULE),
    ExtraProperties = #{<<"ip_replacement_allowed">> => <<"true">>},
    {ok, Token} = yapi_ct_helper:issue_token([{[invoices], write}], unlimited, ExtraProperties),
    yapi_ct_helper:mock_services([
        {invoicing, fun('Create', _) -> {ok, ?PAYPROC_INVOICE} end},
        {bender,    fun('GenerateID', _) -> {ok, yapi_ct_helper_bender:get_result(<<"bender_key">>)} end}
    ], MockServiceSup),
    Req = #{
        <<"shopID">> => ?STRING,
        <<"amount">> => ?INTEGER,
    ...
    } = yapi_client_invoices:create_invoice(yapi_ct_helper:get_context(Token, ExtraProperties), Req),
    yapi_ct_helper:stop_mocked_service_sup(MockServiceSup),
    [{context, yapi_ct_helper:get_context(InvAccToken)} | Config];

init_per_group(operations_by_invoice_access_token_after_token_creation, Config) ->
    MockServiceSup = yapi_ct_helper:start_mocked_service_sup(?MODULE),
    {ok, Token} = yapi_ct_helper:issue_token([{[invoices], write}], unlimited),
    yapi_ct_helper:mock_services([
        {invoicing, fun('Get', _) -> {ok, ?PAYPROC_INVOICE} end},
        {bender,    fun('GenerateID', _) -> {ok, yapi_ct_helper_bender:get_result(<<"bender_key">>)} end}
    ], MockServiceSup),
    {ok, #{<<"payload">> := InvAccToken}
    } = yapi_client_invoices:create_invoice_access_token(yapi_ct_helper:get_context(Token), ?STRING),
    yapi_ct_helper:stop_mocked_service_sup(MockServiceSup),
    [{context, yapi_ct_helper:get_context(InvAccToken)} | Config];
...
create_visa_payment_resource_ok_test(Config) ->
    ...
    }}} = yapi_client_tokens:create_payment_resource(?config(context, Config), #{
        <<"paymentTool">> => #{
...
create_visa_payment_resource_idemp_ok_test(Config) ->
    ...
    }} = yapi_client_tokens:create_payment_resource(?config(context, Config), Params),
    {ok, #{
        <<"paymentToolToken">> := ToolToken,
        <<"paymentSession">>   := ToolSession,
        <<"paymentToolDetails">> := PaymentToolDetails
...
create_visa_payment_resource_idemp_fail_test(Config) ->
    ...
    Token1 = <<"TOKEN1">>,
    Token2 = <<"TOKEN2">>,
    Ctx = yapi_msgp_marshalling:marshal(#{<<"params_hash">> => erlang:phash2(Token1)}),
    yapi_ct_helper:mock_services([
        {cds_storage, fun
            ('PutSession', _) -> {ok, ok};
            ('PutCard', [
    ...
    {ok, _} = yapi_client_tokens:create_payment_resource(?config(context, Config), Params),
    {error, {409, #{
        <<"externalID">> := ExternalID,
        <<"message">>    := <<"This 'externalID' has been used by another request">>
    }}} = yapi_client_tokens:create_payment_resource(?config(context, Config), Params2).
...
ip_replacement_not_allowed_test(Config) ->
    ...
    {ok, Res} = yapi_client_tokens:create_payment_resource(?config(context, Config), #{
        <<"paymentTool">> => #{
            <<"paymentToolType">> => <<"DigitalWalletData">>,
            <<"digitalWalletType">> => <<"DigitalWalletQIWI">>,
            <<"phoneNumber">> => <<"+79876543210">>
...
get_invoice_ok_test(Config) ->
    ...
    {ok, _} = yapi_client_invoices:get_invoice_by_id(?config(context, Config), ?STRING).
...
get_invoice_events_ok_test(Config) ->
    ...
    _ = yapi_ct_helper:mock_services([
        {invoicing, fun
    ...
    {ok, [#{<<"id">> := 1}, #{<<"id">> := 2}, #{<<"id">> := 4}]} =
        yapi_client_invoices:get_invoice_events(?config(context, Config), ?STRING, 3),
    {ok, [#{<<"id">> := 4}, #{<<"id">> := 7}]} =
        yapi_client_invoices:get_invoice_events(?config(context, Config), ?STRING, 2, 3).

-spec get_invoice_payment_methods_ok_test(config()) ->
    _.
get_invoice_payment_methods_ok_test(Config) ->
    yapi_ct_helper:mock_services([{invoicing, fun('ComputeTerms', _) -> {ok, ?TERM_SET} end}], Config),
    {ok, _} = yapi_client_invoices:get_invoice_payment_methods(?config(context, Config), ?STRING).
...
create_payment_ok_test(Config) ->
    ...
    }} = yapi_client_tokens:create_payment_resource(?config(context, Config), Req1),
    Req2 = #{
        <<"externalID">> => ExternalID,
    ...
    {ok, #{<<"id">> := BenderKey, <<"externalID">> := ExternalID}} =
        yapi_client_payments:create_payment(?config(context, Config), Req2, ?STRING).
...
create_payment_with_empty_cvv_ok_test(Config) ->
    ...
    {ok, _} = yapi_client_payments:create_payment(?config(context, Config), Req2, ?STRING).
...
get_payments_ok_test(Config) ->
    yapi_ct_helper:mock_services([{invoicing, fun('Get', _) -> {ok, ?PAYPROC_INVOICE} end}], Config),
    {ok, _} = yapi_client_payments:get_payments(?config(context, Config), ?STRING).
...
get_payment_by_id_ok_test(Config) ->
    ...
    {ok, _} = yapi_client_payments:get_payment_by_id(?config(context, Config), ?STRING, ?STRING).
...
cancel_payment_ok_test(Config) ->
    yapi_ct_helper:mock_services([{invoicing, fun('CancelPayment', _) -> {ok, ok} end}], Config),
    ok = yapi_client_payments:cancel_payment(?config(context, Config), ?STRING, ?STRING, ?STRING).

-spec capture_payment_ok_test(config()) ->
    _.
capture_payment_ok_test(Config) ->
    yapi_ct_helper:mock_services([{invoicing, fun('CapturePaymentNew', _) -> {ok, ok} end}], Config),
    Req = #{
        <<"reason">> => ?STRING
    },
    ok = yapi_client_payments:capture_payment(?config(context, Config), Req, ?STRING, ?STRING).
...
get_recurrent_payments_ok_test(Config) ->
    ...
    yapi_ct_helper:mock_services([{invoicing, fun('Get', _) -> {ok, Invoice} end}], Config),
    {ok, _} = yapi_client_payments:get_payments(?config(context, Config), ?STRING).
