...
get_party_params(Context) ->
    #payproc_PartyParams{
        contact_info = #domain_PartyContactInfo{
            email = anapi_auth:get_claim(<<"email">>, get_auth_context(Context))
        }
    }.
...
get_extra_properties(Context) ->
    anapi_auth:get_claims(get_auth_context(Context)).
%% Common functions
...
issue_access_token(PartyID, TokenSpec, ExtraProperties) ->
    #{<<"payload">> => anapi_auth:issue_access_token(PartyID, TokenSpec, ExtraProperties)}.
...
unwrap_payment_session(Encoded) ->
    #{
        ...
        <<"paymentSession">> := PaymentSession
            ...
            anapi_utils:base64url_to_map(Encoded)
        catch
            error:badarg ->
