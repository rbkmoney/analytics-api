...
process_request('GetClaims', Req, Context) ->
    ...
    Claims = anapi_utils:unwrap(
        anapi_handler_utils:service_call_with([user_info, party_id, party_creation], Call, Context)
    ),
...
process_request('GetClaimByID', Req, Context) ->
    Call = {
        party_management,
        'GetClaim',
        [anapi_handler_utils:get_party_id(Context), genlib:to_int(maps:get('claimID', Req))]
    },
    case anapi_handler_utils:service_call_with([user_info, party_creation], Call, Context) of
        {ok, Claim} ->
...
process_request('CreateClaim', Req, Context) ->
    try
        ...
        Call = {party_management, 'CreateClaim', [anapi_handler_utils:get_party_id(Context), Changeset]},
        case anapi_handler_utils:service_call_with([user_info, party_creation], Call, Context) of
            {ok, Claim} ->
...
%         ]},
%     Party = anapi_utils:unwrap(
%         anapi_handler_utils:service_call_with([user_info, party_id, party_creation], Call, Context)
%     ),
%     {ok, {200, #{}, anapi_handler_utils:anapi_handler_decoder_party:decode_party(Party)}};
    ...
    EncodedMod = case maps:get(<<"shopModificationType">>, Modification) of
        ...
        <<"ShopAccountCreation">> ->
            {shop_account_creation, #payproc_ShopAccountParams{
                currency = anapi_handler_encoder:encode_currency(maps:get(<<"currency">>, Modification))
            }};
        <<"ShopCategoryChange">> ->
...
encode_payout_tool_params(#{<<"currency">> := Currency, <<"details">> := Details}) ->
    #payproc_PayoutToolParams{
        currency = anapi_handler_encoder:encode_currency(Currency),
...
encode_international_bank_details(Acc) ->
    #domain_InternationalBankDetails{
        ...
        country = anapi_handler_encoder:encode_residence(genlib_map:get(<<"countryCode">>, Acc)),
...
encode_private_entity(#{<<"entityType">> := <<"RussianPrivateEntity">>} = Entity) ->
    {russian_private_entity, #domain_RussianPrivateEntity{
        ...
        contact_info = anapi_handler_encoder:encode_contact_info(maps:get(<<"contactInfo">>, Entity))
    }}.
...
decode_contract_modification({creation, ContractParams}) ->
    #{
        ...
        <<"contractor"              >> =>
            anapi_handler_decoder_party:decode_contractor(ContractParams#payproc_ContractParams.contractor),
        <<"paymentInstitutionID"    >> =>
            anapi_handler_decoder_party:decode_payment_institution_ref(
...
decode_contract_modification({legal_agreement_binding, LegalAgreement}) ->
    #{
        ...
        <<"legalAgreement"          >> => anapi_handler_decoder_party:decode_legal_agreement(LegalAgreement)
    };
...
decode_contract_modification({payout_tool_modification, #payproc_PayoutToolModificationUnit{
    ...
    #{
        ...
        <<"payoutToolID"            >> => PayoutToolID,
        <<"details"                 >> => anapi_handler_decoder_party:decode_payout_tool_details(ToolInfo)
    };
decode_contract_modification({report_preferences_modification, ReportPreferences}) ->
    maps:merge(
        ...
        anapi_handler_decoder_party:decode_reporting_preferences(ReportPreferences)
    ).
...
decode_shop_modification({category_modification, CategoryRef}) ->
    #{
        ...
        <<"categoryID"          >> => anapi_handler_decoder_utils:decode_category_ref(CategoryRef)
    };
decode_shop_modification({location_modification, Location}) ->
    #{
        <<"shopModificationType">> => <<"ShopLocationChange">>,
        <<"location"            >> => anapi_handler_decoder_party:decode_shop_location(Location)
    };
decode_shop_modification({details_modification, Details}) ->
    #{
        <<"shopModificationType">> => <<"ShopDetailsChange">>,
        <<"details"             >> => anapi_handler_decoder_party:decode_shop_details(Details)
    };
...
decode_shop_modification({payout_schedule_modification, #payproc_ScheduleModification{schedule = ScheduleRef}}) ->
    genlib_map:compact(#{
        ...
        <<"scheduleID"          >> => anapi_handler_decoder_utils:decode_business_schedule_ref(ScheduleRef)
    }).

decode_payout_tool_params(#payproc_PayoutToolParams{currency = Currency, tool_info = ToolInfo}) ->
    anapi_handler_decoder_party:decode_payout_tool_params(Currency, ToolInfo).

decode_shop_params(ShopParams) ->
    #{
        <<"location"    >> => anapi_handler_decoder_party:decode_shop_location(ShopParams#payproc_ShopParams.location),
        <<"details"     >> => anapi_handler_decoder_party:decode_shop_details(ShopParams#payproc_ShopParams.details),
