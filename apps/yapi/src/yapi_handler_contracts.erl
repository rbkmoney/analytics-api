...
decode_contracts_map(Contracts, Contractors) ->
    yapi_handler_decoder_utils:decode_map(Contracts, fun(C) -> decode_contract(C, Contractors) end).
...
decode_contract(Contract, Contractors) ->
    yapi_handler_utils:merge_and_compact(#{
        ...
        <<"contractor"          >> => yapi_handler_decoder_party:decode_contractor(
            get_contractor(Contract, Contractors)
        ),
        <<"paymentInstitutionID">> =>
            yapi_handler_decoder_party:decode_payment_institution_ref(Contract#domain_Contract.payment_institution),
        <<"validSince"          >> => Contract#domain_Contract.valid_since,
        <<"validUntil"          >> => Contract#domain_Contract.valid_until,
        <<"legalAgreement"      >> => yapi_handler_decoder_utils:decode_optional(
            Contract#domain_Contract.legal_agreement,
            fun yapi_handler_decoder_party:decode_legal_agreement/1
        ),
        <<"reportingPreferences">> => yapi_handler_decoder_utils:decode_optional(
            Contract#domain_Contract.report_preferences,
            fun yapi_handler_decoder_party:decode_reporting_preferences/1
        )
