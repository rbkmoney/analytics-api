        ...
        {authorizers, #{
            jwt => #{
                ...
                keyset => #{
                    ...
                    yapi => {pem_file, get_keysource("keys/local/private.pem", Config)}
                }
            }
        }}
    ],
    start_app(yapi, CapiEnv).
...
issue_token(PartyID, ACL, LifeTime, ExtraProperties) ->
    ...
    yapi_authorizer_jwt:issue({{PartyID, yapi_acl:from_list(ACL)}, Claims}, LifeTime).

-spec get_context(binary()) ->
    yapi_client_lib:context().
...
get_context(Token, ExtraProperties) ->
    yapi_client_lib:get_context(?CAPI_URL, Token, 10000, ipv4, ExtraProperties).

% TODO move it to `yapi_dummy_service`, looks more appropriate
...
start_woody_client(ServiceURLs) ->
    start_app(yapi_woody_client, [{service_urls, ServiceURLs}]).
...
mock_services_(Services, SupPid) when is_pid(SupPid) ->
    ...
    ChildSpec = woody_server:child_spec(
        ...
        #{
            ip => IP,
            port => Port,
            event_handler => yapi_woody_event_handler,
...
mock_service_handler({ServiceName, Fun}) ->
    mock_service_handler(ServiceName, yapi_woody_client:get_service_modname(ServiceName), Fun);
mock_service_handler({ServiceName, WoodyService, Fun}) ->
    mock_service_handler(ServiceName, WoodyService, Fun).

mock_service_handler(ServiceName, WoodyService, Fun) ->
    {make_path(ServiceName), {WoodyService, {yapi_dummy_service, #{function => Fun}}}}.
