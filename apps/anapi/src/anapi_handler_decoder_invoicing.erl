...
-spec decode_payment(binary(), anapi_handler_encoder:encode_data(), processing_context()) ->
    anapi_handler_decoder_utils:decode_data().
...
decode_payer({customer, #domain_CustomerPayer{customer_id = ID, payment_tool = PaymentTool}}) ->
    #{
        ...
        <<"customerID">> => ID,
        <<"paymentToolToken">> => anapi_handler_decoder_party:decode_payment_tool_token(PaymentTool),
        <<"paymentToolDetails">> => anapi_handler_decoder_party:decode_payment_tool_details(PaymentTool)
    };
decode_payer({recurrent, RecurrentPayer}) ->
    ...
    #{
        ...
        <<"contactInfo">> => anapi_handler_decoder_party:decode_contact_info(ContactInfo),
...
decode_payer({payment_resource, #domain_PaymentResourcePayer{resource = Resource, contact_info = ContactInfo}}) ->
    maps:merge(
        ...
        anapi_handler_decoder_party:decode_disposable_payment_resource(Resource)
    ).
...
-spec decode_refund(anapi_handler_encoder:encode_data(), processing_context()) ->
    anapi_handler_decoder_utils:decode_data().
...
-spec decode_invoice(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
decode_invoice(Invoice) ->
    ...
    anapi_handler_utils:merge_and_compact(#{
        ...
        <<"amount"           >> => Amount,
        <<"currency"         >> => anapi_handler_decoder_utils:decode_currency(Currency),
        <<"metadata"         >> => anapi_handler_decoder_utils:decode_context(Invoice#domain_Invoice.context),
        <<"product"          >> => Product,
...
compute_terms(ServiceName, Args, Context) ->
    anapi_handler_utils:service_call_with([user_info], {ServiceName, 'ComputeTerms', Args}, Context).

-spec make_invoice_and_token(anapi_handler_encoder:encode_data(), binary()) ->
    anapi_handler_decoder_utils:decode_data().

make_invoice_and_token(Invoice, PartyID) ->
    make_invoice_and_token(Invoice, PartyID, #{}).

-spec make_invoice_and_token(anapi_handler_encoder:encode_data(), binary(), map()) ->
    anapi_handler_decoder_utils:decode_data().

make_invoice_and_token(Invoice, PartyID, ExtraProperties) ->
    #{
        <<"invoice"           >> => decode_invoice(Invoice),
        <<"invoiceAccessToken">> => anapi_handler_utils:issue_access_token(
            PartyID,
