-module(anapi_handler_invoice_templates).
...
process_request('CreateInvoiceWithTemplate' = OperationID, Req, Context) ->
    ...
    try create_invoice(PartyID, InvoiceTplID, InvoiceParams, Context, OperationID) of
        {ok, #'payproc_Invoice'{invoice = Invoice}} ->
            {ok, {201, #{}, anapi_handler_decoder_invoicing:make_invoice_and_token(
                Invoice, anapi_handler_utils:get_party_id(Context), ExtraProperties)
            }};
        {exception, Exception} ->
            case Exception of
                #payproc_InvalidUser{} ->
...
process_request('GetInvoicePaymentMethodsByTemplateID', Req, Context) ->
    Result =
        anapi_handler_decoder_invoicing:construct_payment_methods(
            invoice_templating,
            [maps:get('invoiceTemplateID', Req), anapi_utils:unwrap(rfc3339:format(erlang:system_time()))],
            Context
        ),
    case Result of
...
encode_invoice_tpl_create_params(PartyID, Params) ->
    ...
    #payproc_InvoiceTemplateCreateParams{
        ...
        details          = Details,
        context          = anapi_handler_encoder:encode_invoice_context(Params)
    }.
...
make_invoice_tpl_and_token(InvoiceTpl, PartyID, ExtraProperties) ->
    #{
        ...
        <<"invoiceTemplateAccessToken">> =>
            anapi_handler_utils:issue_access_token(
                PartyID,
...
encode_invoice_tpl_details(#{<<"templateType">> := <<"InvoiceTemplateMultiLine">>} = Details) ->
    {cart, anapi_handler_encoder:encode_invoice_cart(Details)};
...
encode_optional_context(Params = #{<<"metadata">> := _}) ->
    anapi_handler_encoder:encode_invoice_context(Params);
...
encode_invoice_tpl_product(Details) ->
    #domain_InvoiceTemplateProduct{
        ...
        metadata = anapi_handler_encoder:encode_invoice_line_meta(Details)
    }.

encode_optional_invoice_cost(Params = #{<<"amount">> := _, <<"currency">> := _}) ->
    anapi_handler_encoder:encode_cash(Params);
...
decode_invoice_tpl(InvoiceTpl) ->
    ...
    genlib_map:compact(#{
        ...
        <<"metadata"   >> => anapi_handler_decoder_utils:decode_context(InvoiceTpl#domain_InvoiceTemplate.context)
    }).
...
decode_invoice_tpl_details({cart, Cart}) ->
    #{
        ...
        <<"cart"        >> => anapi_handler_decoder_invoicing:decode_invoice_cart(Cart)
    };
decode_invoice_tpl_details({product, Product}) ->
    genlib_map:compact(#{
        ...
        <<"taxMode"     >> => anapi_handler_decoder_invoicing:decode_invoice_line_tax_mode(
...
get_currency_from_cart(#domain_InvoiceCart{lines = [FirstLine | _]}) ->
    ...
    anapi_handler_decoder_utils:decode_currency(Currency).
