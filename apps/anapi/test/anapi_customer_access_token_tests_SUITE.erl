-module(anapi_customer_access_token_tests_SUITE).
...
init_per_group(operations_by_customer_access_token_after_customer_creation, Config) ->
    ...
    {ok, Token} = anapi_ct_helper:issue_token([{[customers], write}], unlimited),
    anapi_ct_helper:mock_services([{customer_management, fun('Create', _) -> {ok, ?CUSTOMER} end}], MockServiceSup),
    Req = #{
        <<"shopID">> => ?STRING,
    ...
    } = anapi_client_customers:create_customer(anapi_ct_helper:get_context(Token), Req),
    anapi_ct_helper:stop_mocked_service_sup(MockServiceSup),
    [{context, anapi_ct_helper:get_context(CustAccToken)} | Config];

init_per_group(operations_by_customer_access_token_after_token_creation, Config) ->
    MockServiceSup = anapi_ct_helper:start_mocked_service_sup(?MODULE),
    {ok, Token} = anapi_ct_helper:issue_token([{[customers], write}], unlimited),
    anapi_ct_helper:mock_services([{customer_management, fun('Get', _) -> {ok, ?CUSTOMER} end}], MockServiceSup),
    {ok,
        #{<<"payload">> := CustAccToken}
    } = anapi_client_customers:create_customer_access_token(anapi_ct_helper:get_context(Token), ?STRING),
    anapi_ct_helper:stop_mocked_service_sup(MockServiceSup),
    [{context, anapi_ct_helper:get_context(CustAccToken)}| Config];
...
get_customer_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_customers:get_customer_by_id(?config(context, Config), ?STRING).
...
create_binding_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_customers:create_binding(?config(context, Config), ?STRING, Req2).
...
get_bindings_ok_test(Config) ->
    anapi_ct_helper:mock_services([{customer_management, fun('Get', _) -> {ok, ?CUSTOMER} end}], Config),
    {ok, _} = anapi_client_customers:get_bindings(?config(context, Config), ?STRING).

-spec get_binding_ok_test(config()) ->
    _.
get_binding_ok_test(Config) ->
    anapi_ct_helper:mock_services([{customer_management, fun('Get', _) -> {ok, ?CUSTOMER} end}], Config),
    {ok, _} = anapi_client_customers:get_binding(?config(context, Config), ?STRING, ?STRING).

-spec get_customer_events_ok_test(config()) ->
    _.
get_customer_events_ok_test(Config) ->
    anapi_ct_helper:mock_services([{customer_management, fun('GetEvents', _) -> {ok, []} end}], Config),
    {ok, _} = anapi_client_customers:get_customer_events(?config(context, Config), ?STRING, 10).
