...
-type subject()    :: {subject_id(), anapi_acl:t()}.
...
construct_final_claims({{Subject, ACL}, Claims}, Expiration) ->
    maps:merge(
        ...
        encode_roles(anapi_acl:encode(ACL))
    ).
...
verify(Token) ->
    try
        ...
        Header = anapi_utils:base64url_to_map(ProtectedHeader),
        Alg = get_alg(Header),
        KID = get_kid(Header),
...
get_result(SubjectID, {Roles, Claims}) ->
    try
        Subject = {SubjectID, anapi_acl:decode(Roles)},
        {ok, {Subject, Claims}}
    catch
        error:{badarg, _} = Reason ->
