...
get_idempotent_key(Prefix, PartyID, ExternalID) ->
    <<"anapi/", Prefix/binary, "/", PartyID/binary, "/", ExternalID/binary>>.
...
generate_id([Key, BenderSchema, Hash], WoodyCtx) ->
    Context = anapi_msgp_marshalling:marshal(#{
        <<"version">>     => ?SCHEMA_VER1,
        <<"params_hash">> => Hash
    }),
    Args = [Key, BenderSchema, Context],
    Result = case anapi_woody_client:call_service(bender, 'GenerateID', Args, WoodyCtx) of
        ...
        {ok, #bender_GenerationResult{internal_id = InternalID, context = Ctx}}       ->
            #{<<"params_hash">> := BenderHash} = anapi_msgp_marshalling:unmarshal(Ctx),
