-module(anapi_woody_tests_SUITE).
...
woody_unexpected_test(Config) ->
    _ = anapi_ct_helper:mock_services([{party_management, fun('Get', _) -> {ok, "spanish inquisition"} end}], Config),
    ?badresp(500) = anapi_client_parties:get_my_party(?config(context, Config)).

-spec woody_unavailable_test(config()) ->
    _.

woody_unavailable_test(Config) ->
    _ = anapi_ct_helper:start_app(anapi_woody_client, [{service_urls, #{
        party_management => <<"http://spanish.inquision/v1/partymgmt">>
    }}]),
    ?badresp(503) = anapi_client_parties:get_my_party(?config(context, Config)).

-spec woody_retry_test(config()) ->
    _.

woody_retry_test(Config) ->
    _ = anapi_ct_helper:start_app(anapi_woody_client, [
        {service_urls, #{
            party_management => <<"http://spanish.inquision/v1/partymgmt">>
        }},
        {service_retries, #{
    ...
    {Time, ?badresp(503)} = timer:tc(anapi_client_parties, get_my_party, [?config(context, Config)]),
...
woody_unknown_test(Config) ->
    _ = anapi_ct_helper:mock_services([{party_management, fun('Get', _) -> timer:sleep(60000) end}], Config),
    ?badresp(504) = anapi_client_parties:get_my_party(?config(context, Config)).
