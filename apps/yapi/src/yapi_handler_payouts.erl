...
process_request('GetPayoutTools', Req, Context) ->
    case yapi_handler_utils:get_contract_by_id(maps:get('contractID', Req), Context) of
...
process_request('GetPayout', Req, Context) ->
    ...
    case yapi_handler_utils:service_call({payouts, 'Get', [PayoutID]}, Context) of
        {ok, Payout} ->
            case check_party_in_payout(yapi_handler_utils:get_party_id(Context), Payout) of
                true ->
...
process_request('CreatePayout', Req, Context) ->
    ...
    case yapi_handler_utils:service_call({payouts, 'CreatePayout', [CreateRequest]}, Context) of
        {ok, Payout} ->
...
get_schedule_by_id(ScheduleID, #{woody_context := WoodyContext}) ->
    ...
    yapi_domain:get(Ref, WoodyContext).

%%

encode_msgpack_value(_Key, Value) ->
    yapi_msgpack:wrap(Value).
...
encode_payout_params(PartyID, PayoutParams) ->
    #'payout_processing_PayoutParams'{
        ...
        amount = yapi_handler_encoder:encode_cash(
...
decode_payout_tool(#domain_PayoutTool{id = ID, currency = Currency, payout_tool_info = Info}) ->
    maps:merge(
        #{<<"id">> => ID},
        yapi_handler_decoder_party:decode_payout_tool_params(Currency, Info)
    ).
...
decode_payout(Payout) ->
    yapi_handler_utils:merge_and_compact(#{
        ...
        <<"currency"         >> => yapi_handler_decoder_utils:decode_currency(Payout#payout_processing_Payout.currency),
...
decode_payout_tool_details(PayoutType) ->
    yapi_handler_decoder_party:decode_payout_tool_details(payout_proc_to_domain(PayoutType)).
