...
-type subject()    :: {subject_id(), yapi_acl:t()}.
...
construct_final_claims({{Subject, ACL}, Claims}, Expiration) ->
    maps:merge(
        ...
        encode_roles(yapi_acl:encode(ACL))
    ).
...
verify(Token) ->
    try
        ...
        Header = yapi_utils:base64url_to_map(ProtectedHeader),
        Alg = get_alg(Header),
        KID = get_kid(Header),
...
get_result(SubjectID, {Roles, Claims}) ->
    try
        Subject = {SubjectID, yapi_acl:decode(Roles)},
        {ok, {Subject, Claims}}
    catch
        error:{badarg, _} = Reason ->
