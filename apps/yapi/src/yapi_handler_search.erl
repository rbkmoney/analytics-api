...
process_request('SearchInvoices', Req, Context) ->
    Query = #{
        <<"merchant_id"              >> => yapi_handler_utils:get_party_id(Context),
...
process_search_request(QueryType, Query, Req, Context, Opts = #{thrift_fun := ThriftFun}) ->
    ...
    Call = {
        ...
        ThriftFun,
        [
            yapi_handler_encoder:encode_stat_request(
                yapi_handler_utils:create_dsl(QueryType, Query, QueryParams),
                ContinuationToken
            )
        ]
    },
    process_search_request_result(QueryType, yapi_handler_utils:service_call(Call, Context), Context, Opts).
...
decode_stat_invoice(Invoice, _Context) ->
    yapi_handler_utils:merge_and_compact(#{
        ...
        <<"metadata"   >> => yapi_handler_decoder_utils:decode_context(Invoice#merchstat_StatInvoice.context),
        <<"product"    >> => Invoice#merchstat_StatInvoice.product,
        <<"description">> => Invoice#merchstat_StatInvoice.description,
        <<"cart"       >> => yapi_handler_decoder_invoicing:decode_invoice_cart(Invoice#merchstat_StatInvoice.cart)
...
decode_stat_payment(Stat, Context) ->
    yapi_handler_utils:merge_and_compact(#{
        ...
        <<"metadata"       >> => yapi_handler_decoder_utils:decode_context(Stat#merchstat_StatPayment.context),
        <<"transactionInfo">> => decode_stat_tx_info(Stat#merchstat_StatPayment.additional_transaction_info),
        <<"makeRecurrent"  >> => yapi_handler_decoder_invoicing:decode_make_recurrent(
...
decode_stat_payer({recurrent, RecurrentPayer}) ->
    ...
    #{
        ...
        <<"recurrentParentPayment">> => yapi_handler_decoder_invoicing:decode_recurrent_parent(RecurrentParent),
        <<"paymentToolToken">> => decode_stat_payment_tool_token(PaymentTool),
        <<"paymentToolDetails">> => decode_stat_payment_tool_details(PaymentTool)
    };
    ...
    Error =
        case StatusInfo of
            #merchstat_InvoicePaymentFailed{failure = OperationFailure} ->
                yapi_handler_decoder_invoicing:decode_payment_operation_failure(OperationFailure, Context);
            _ ->
                undefined
        end,
    #{
...
decode_bank_card_details(BankCard, V) ->
    LastDigits = yapi_handler_decoder_utils:decode_last_digits(BankCard#merchstat_BankCard.masked_pan),
...
decode_international_bank_details(Bank) ->
    genlib_map:compact(#{
         ...
         <<"countryCode">> =>
            yapi_handler_decoder_party:decode_residence(Bank#merchstat_InternationalBankDetails.country),
