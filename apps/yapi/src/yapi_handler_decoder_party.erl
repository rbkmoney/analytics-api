-module(yapi_handler_decoder_party).
...
-spec decode_shop_location(yapi_handler_encoder:encode_data()) ->
...
-spec decode_shop_details(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec decode_contact_info(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec decode_party(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec decode_contractor(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec decode_legal_agreement(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec decode_reporting_preferences(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec decode_payment_institution_ref(yapi_handler_encoder:encode_data()) ->
    integer().

decode_payment_institution_ref(#domain_PaymentInstitutionRef{id = Ref}) ->
    Ref.

-spec decode_payment_tool_token(yapi_handler_encoder:encode_data()) ->
    binary().
...
decode_bank_card(#domain_BankCard{
    ...
    yapi_utils:map_to_base64url(genlib_map:compact(#{
        <<"type"          >> => <<"bank_card">>,
        <<"token"         >> => Token,
        <<"payment_system">> => PaymentSystem,
        <<"bin"           >> => Bin,
...
decode_bank_card_metadata(Meta) ->
    maps:map(fun(_, Data) -> yapi_msgp_marshalling:unmarshal(Data) end, Meta).

decode_payment_terminal(#domain_PaymentTerminal{
    terminal_type = Type
}) ->
    yapi_utils:map_to_base64url(#{
...
-spec decode_payment_tool_details(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
decode_bank_card_details(BankCard, V) ->
    LastDigits = yapi_handler_decoder_utils:decode_last_digits(BankCard#domain_BankCard.masked_pan),
    Bin = BankCard#domain_BankCard.bin,
    yapi_handler_utils:merge_and_compact(V, #{
        <<"lastDigits">>     => LastDigits,
        <<"bin">>            => Bin,
        <<"cardNumberMask">> => yapi_handler_decoder_utils:decode_masked_pan(Bin, LastDigits),
...
mask_phone_number(PhoneNumber) ->
    yapi_utils:redact(PhoneNumber, <<"^\\+\\d(\\d{1,10}?)\\d{2,4}$">>).

-spec decode_disposable_payment_resource(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
decode_disposable_payment_resource(Resource) ->
    ...
    #{
        ...
        <<"paymentSession"    >> => yapi_handler_utils:wrap_payment_session(ClientInfo, SessionID),
...
-spec decode_payout_tool_params(yapi_handler_encoder:encode_data(), {atom(), _}) ->
    yapi_handler_decoder_utils:decode_data().
