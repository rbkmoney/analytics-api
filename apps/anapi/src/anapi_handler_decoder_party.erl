...
-spec decode_shop_location(anapi_handler_encoder:encode_data()) ->
...
-spec decode_shop_details(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
-spec decode_contact_info(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
-spec decode_party(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
-spec decode_contractor(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
-spec decode_legal_agreement(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
-spec decode_reporting_preferences(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
-spec decode_payment_institution_ref(anapi_handler_encoder:encode_data()) ->
    integer().

decode_payment_institution_ref(#domain_PaymentInstitutionRef{id = Ref}) ->
    Ref.

-spec decode_payment_tool_token(anapi_handler_encoder:encode_data()) ->
    binary().
...
decode_bank_card_metadata(Meta) ->
    maps:map(fun(_, Data) -> anapi_msgp_marshalling:unmarshal(Data) end, Meta).
...
-spec decode_payment_tool_details(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
decode_bank_card_details(BankCard, V) ->
    LastDigits = anapi_handler_decoder_utils:decode_last_digits(BankCard#domain_BankCard.masked_pan),
...
-spec decode_disposable_payment_resource(anapi_handler_encoder:encode_data()) ->
    anapi_handler_decoder_utils:decode_data().
...
decode_disposable_payment_resource(Resource) ->
    ...
    #{
        ...
        <<"paymentSession"    >> => anapi_handler_utils:wrap_payment_session(ClientInfo, SessionID),
...
-spec decode_payout_tool_params(anapi_handler_encoder:encode_data(), {atom(), _}) ->
    anapi_handler_decoder_utils:decode_data().
