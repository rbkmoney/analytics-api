...
process_request('GetPaymentByID', Req, Context) ->
    InvoiceID = maps:get(invoiceID, Req),
    case yapi_handler_utils:get_payment_by_id(InvoiceID, maps:get(paymentID, Req), Context) of
        {ok, Payment} ->
...
process_request('CreateRefund', Req, Context) ->
    ...
    case yapi_handler_utils:service_call_with([user_info], Call, Context) of
        {ok, Refund} ->
            {ok, {201, #{}, yapi_handler_decoder_invoicing:decode_refund(Refund, Context)}};
        {exception, Exception} ->
            case Exception of
                #payproc_InvalidUser{} ->
                    {ok, general_error(404, <<"Invoice not found">>)};
...
process_request('GetRefunds', Req, Context) ->
    case yapi_handler_utils:get_payment_by_id(maps:get(invoiceID, Req), maps:get(paymentID, Req), Context) of
        {ok, #payproc_InvoicePayment{refunds = Refunds}} ->
            {ok, {200, #{}, [yapi_handler_decoder_invoicing:decode_refund(R, Context) || R <- Refunds]}};
        {exception, Exception} ->
            case Exception of
                #payproc_InvalidUser{} ->
                    {ok, general_error(404, <<"Invoice not found">>)};
...
process_request('GetRefundByID', Req, Context) ->
    ...
    case yapi_handler_utils:service_call_with([user_info], Call, Context) of
        {ok, Refund} ->
            {ok, {200, #{}, yapi_handler_decoder_invoicing:decode_refund(Refund, Context)}};
        {exception, Exception} ->
            case Exception of
...
create_payment(InvoiceID, PartyID, PaymentParams, #{woody_context := WoodyCtx} = Context, BenderPrefix) ->
    ...
    case yapi_bender:gen_by_sequence(IdempotentKey, InvoiceID, Hash, WoodyCtx) of
        {ok, ID} ->
            Params = encode_invoice_payment_params(ID, ExternalID, PaymentParams),
            Call = {invoicing, 'StartPayment', [InvoiceID, Params]},
            yapi_handler_utils:service_call_with([user_info], Call, Context);
...
encode_payer_params(#{
    ...
    PaymentTool = yapi_handler_encoder:encode_payment_tool_token(Token),
    {ClientInfo, PaymentSession} = yapi_handler_utils:unwrap_payment_session(EncodedSession),
    {payment_resource, #payproc_PaymentResourcePayerParams{
        ...
        contact_info = yapi_handler_encoder:encode_contact_info(ContactInfo)
    }};
...
encode_optional_cash(Params = #{<<"amount">> := _}, InvoiceID, PaymentID, Context) ->
    ...
    yapi_handler_encoder:encode_cash(Params#{<<"currency">> => yapi_handler_decoder_utils:decode_currency(Currency)});
...
decode_invoice_payment(InvoiceID, #payproc_InvoicePayment{payment = Payment}, Context) ->
    yapi_handler_decoder_invoicing:decode_payment(InvoiceID, Payment, Context).
