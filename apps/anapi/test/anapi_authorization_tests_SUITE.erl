-module(anapi_authorization_tests_SUITE).
...
authorization_positive_lifetime_ok_test(_Config) ->
    {ok, Token} = anapi_ct_helper:issue_token([], {lifetime, 10}),
    {ok, _} = anapi_client_categories:get_categories(anapi_ct_helper:get_context(Token)).

-spec authorization_unlimited_lifetime_ok_test(config()) ->
    _.
authorization_unlimited_lifetime_ok_test(_Config) ->
    {ok, Token} = anapi_ct_helper:issue_token([], unlimited),
    {ok, _} = anapi_client_categories:get_categories(anapi_ct_helper:get_context(Token)).

-spec authorization_far_future_deadline_ok_test(config()) ->
    _.
authorization_far_future_deadline_ok_test(_Config) ->
    {ok, Token} = anapi_ct_helper:issue_token([], {deadline, 4102444800}), % 01/01/2100 @ 12:00am (UTC)
    {ok, _} = anapi_client_categories:get_categories(anapi_ct_helper:get_context(Token)).
...
authorization_permission_ok_test(Config) ->
    ...
    {ok, Token} = anapi_ct_helper:issue_token([{[party], read}], unlimited),
    {ok, _} = anapi_client_parties:get_my_party(anapi_ct_helper:get_context(Token)).

-spec authorization_negative_lifetime_error_test(config()) ->
    _.
authorization_negative_lifetime_error_test(_Config) ->
    {ok, Token} = anapi_ct_helper:issue_token([], {lifetime, -10}),
    ?emptyresp(401) = anapi_client_categories:get_categories(anapi_ct_helper:get_context(Token)).

-spec authorization_bad_deadline_error_test(config()) ->
    _.
authorization_bad_deadline_error_test(_Config) ->
    {ok, Token} = anapi_ct_helper:issue_token([], {deadline, -10}),
    ?emptyresp(401) = anapi_client_categories:get_categories(anapi_ct_helper:get_context(Token)).
...
authorization_error_no_permission_test(_Config) ->
    {ok, Token} = anapi_ct_helper:issue_token([], {lifetime, 10}),
    ?emptyresp(401) = anapi_client_parties:get_my_party(anapi_ct_helper:get_context(Token)).
...
issue_dummy_token(ACL, Config) ->
    Claims = #{
        ...
        <<"resource_access">> => #{
            <<"common-api">> => #{
                <<"roles">> => anapi_acl:encode(anapi_acl:from_list(ACL))
            }
        }
    },
    BadPemFile = anapi_ct_helper:get_keysource("keys/local/dummy.pem", Config),
    BadJWK = jose_jwk:from_pem_file(BadPemFile),
    GoodPemFile = anapi_ct_helper:get_keysource("keys/local/private.pem", Config),
