...
create_stat_dsl(StatType, Req, Context) ->
    FromTime = anapi_handler_utils:get_time('fromTime', Req),
    ToTime   = anapi_handler_utils:get_time('toTime'  , Req),
    SplitInterval =
        case StatType of
            customers_rate_stat ->
                anapi_handler_utils:get_time_diff(FromTime, ToTime);
            _ ->
                SplitUnit = genlib_map:get('splitUnit', Req),
                SplitSize = genlib_map:get('splitSize', Req),
                anapi_handler_utils:get_split_interval(SplitSize, SplitUnit)
        end,
    Query = #{
    ...
    anapi_handler_utils:create_dsl(StatType, Query, #{}).

process_merchant_stat(StatType, Req, Context) ->
    Call = {
        merchant_stat,
        'GetStatistics',
        [anapi_handler_encoder:encode_stat_request(create_stat_dsl(StatType, Req, Context))]
    },
    process_merchant_stat_result(StatType, anapi_handler_utils:service_call(Call, Context)).
