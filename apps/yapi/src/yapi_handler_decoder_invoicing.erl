-module(yapi_handler_decoder_invoicing).
...
-type processing_context() :: yapi_handler:processing_context().

%%

-spec decode_user_interaction({atom(), _}) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec decode_payment(binary(), yapi_handler_encoder:encode_data(), processing_context()) ->
    yapi_handler_decoder_utils:decode_data().
...
decode_payer({customer, #domain_CustomerPayer{customer_id = ID, payment_tool = PaymentTool}}) ->
    #{
        ...
        <<"customerID">> => ID,
        <<"paymentToolToken">> => yapi_handler_decoder_party:decode_payment_tool_token(PaymentTool),
        <<"paymentToolDetails">> => yapi_handler_decoder_party:decode_payment_tool_details(PaymentTool)
    };
decode_payer({recurrent, RecurrentPayer}) ->
    ...
    #{
        ...
        <<"contactInfo">> => yapi_handler_decoder_party:decode_contact_info(ContactInfo),
...
decode_payer({payment_resource, #domain_PaymentResourcePayer{resource = Resource, contact_info = ContactInfo}}) ->
    maps:merge(
        ...
        yapi_handler_decoder_party:decode_disposable_payment_resource(Resource)
    ).
...
decode_payment_operation_failure({failure, Failure}, Context) ->
    case yapi_auth:get_consumer(yapi_auth:get_claims(yapi_handler_utils:get_auth_context(Context))) of
        client ->
...
-spec decode_recurrent_parent(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
-spec payment_error_client_maping(yapi_handler_encoder:encode_data()) ->
    binary().
...
-spec decode_refund(yapi_handler_encoder:encode_data(), processing_context()) ->
    yapi_handler_decoder_utils:decode_data().
    ...
    Error =
        case StatusInfo of
            #domain_InvoicePaymentRefundFailed{failure = OperationFailure} ->
                yapi_handler_decoder_utils:decode_operation_failure(OperationFailure, Context);
            _ ->
                undefined
        end,
    #{
...
-spec decode_invoice(yapi_handler_encoder:encode_data()) ->
    yapi_handler_decoder_utils:decode_data().
...
decode_invoice(Invoice) ->
    ...
    yapi_handler_utils:merge_and_compact(#{
        ...
        <<"amount"           >> => Amount,
        <<"currency"         >> => yapi_handler_decoder_utils:decode_currency(Currency),
        <<"metadata"         >> => yapi_handler_decoder_utils:decode_context(Invoice#domain_Invoice.context),
        <<"product"          >> => Product,
...
-spec decode_invoice_cart(yapi_handler_encoder:encode_data() | undefined) ->
    yapi_handler_decoder_utils:decode_data() | undefined.
...
compute_terms(ServiceName, Args, Context) ->
    yapi_handler_utils:service_call_with([user_info], {ServiceName, 'ComputeTerms', Args}, Context).

-spec make_invoice_and_token(yapi_handler_encoder:encode_data(), binary()) ->
    yapi_handler_decoder_utils:decode_data().

make_invoice_and_token(Invoice, PartyID) ->
    make_invoice_and_token(Invoice, PartyID, #{}).

-spec make_invoice_and_token(yapi_handler_encoder:encode_data(), binary(), map()) ->
    yapi_handler_decoder_utils:decode_data().

make_invoice_and_token(Invoice, PartyID, ExtraProperties) ->
    #{
        <<"invoice"           >> => decode_invoice(Invoice),
        <<"invoiceAccessToken">> => yapi_handler_utils:issue_access_token(
            PartyID,
