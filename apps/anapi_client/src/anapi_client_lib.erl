...
-spec prepare_search_param({atom(), term()}) -> map().
prepare_search_param(Param) ->
    case Param of
        {shopID, P} -> #{<<"shopID">> => genlib:to_binary(P)};
        {shopIDs, P} -> #{<<"shopIDs">> => genlib:to_binary(P)};
        {invoiceIDs, P} -> #{<<"invoiceIDs">> => genlib:to_binary(P)};
        {excludeShopIDs, P} -> #{<<"excludeShopIDs">> => genlib:to_binary(P)};
...
-spec prepare_reporting_param({atom(), term()}) -> map().
...
-spec prepare_analytics_param({atom(), term()}) -> map().
