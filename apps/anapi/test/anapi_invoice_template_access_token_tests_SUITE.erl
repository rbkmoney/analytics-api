-module(anapi_invoice_template_access_token_tests_SUITE).
...
init_per_group(operations_by_invoice_template_access_token, Config) ->
    ...
    {ok, Token} = anapi_ct_helper:issue_token([{[party], write}], unlimited),
    anapi_ct_helper:mock_services([{invoice_templating, fun('Create', _) -> {ok, ?INVOICE_TPL} end}], MockServiceSup),
    Req = #{
        <<"shopID">> => ?STRING,
    ...
    } = anapi_client_invoice_templates:create(anapi_ct_helper:get_context(Token), Req),
    anapi_ct_helper:stop_mocked_service_sup(MockServiceSup),
    [{context, anapi_ct_helper:get_context(InvTemplAccToken)} | Config];
...
create_invoice_with_tpl_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_invoice_templates:create_invoice(?config(context, Config), ?STRING, Req).

-spec get_invoice_template_ok_test(config()) ->
    _.
get_invoice_template_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoice_templating, fun('Get', _) -> {ok, ?INVOICE_TPL} end}], Config),
    {ok, _} = anapi_client_invoice_templates:get_template_by_id(?config(context, Config), ?STRING).

-spec get_invoice_payment_methods_by_tpl_id_ok_test(config()) ->
    _.
get_invoice_payment_methods_by_tpl_id_ok_test(Config) ->
    anapi_ct_helper:mock_services([{'invoice_templating', fun('ComputeTerms', _) -> {ok, ?TERM_SET} end}], Config),
    {ok, _} = anapi_client_invoice_templates:get_invoice_payment_methods(?config(context, Config), ?STRING).
