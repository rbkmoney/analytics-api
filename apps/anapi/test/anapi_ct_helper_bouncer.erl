-module(anapi_ct_helper_bouncer).
    ...
    mock_bouncer_arbiter(
        ?assertContextMatches(
            #bctx_v1_ContextFragment{
                anapi = ?CTX_ANAPI(?CTX_PARTY_OP(Op, PartyID))
            }
        ),
        Config
    ).
...
start_bouncer_client(ServiceURLs) ->
    ...
    anapi_ct_helper:start_app(bouncer_client, [{service_clients, maps:merge(Acc, ServiceClients)}]).

-spec mock_bouncer_client(_) -> _.
mock_bouncer_client(SupOrConfig) ->
    start_bouncer_client(
        anapi_ct_helper:mock_services_(
            [
                {
                    org_management,
