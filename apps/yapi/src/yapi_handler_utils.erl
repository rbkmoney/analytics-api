...
-type response()           :: yapi_handler:response().
...
service_call({ServiceName, Function, Args}, #{woody_context := WoodyContext}) ->
    yapi_woody_client:call_service(ServiceName, Function, Args, WoodyContext).

get_party_params(Context) ->
    #payproc_PartyParams{
        contact_info = #domain_PartyContactInfo{
            email = yapi_auth:get_claim(<<"email">>, get_auth_context(Context))
        }
    }.
...
get_party_id(Context) ->
    yapi_auth:get_subject_id(get_auth_context(Context)).

-spec get_extra_properties(processing_context()) ->
    map().

get_extra_properties(Context) ->
    yapi_auth:get_claims(get_auth_context(Context)).
%% Common functions
...
issue_access_token(PartyID, TokenSpec, ExtraProperties) ->
    #{<<"payload">> => yapi_auth:issue_access_token(PartyID, TokenSpec, ExtraProperties)}.
...
get_time(Key, Req) ->
    case genlib_map:get(Key, Req) of
        Timestamp when is_binary(Timestamp) ->
            yapi_utils:to_universal_time(Timestamp);
        undefined ->
            undefined
    end.
...
unwrap_payment_session(Encoded) ->
    #{
        ...
        <<"paymentSession">> := PaymentSession
            ...
            yapi_utils:base64url_to_map(Encoded)
        catch
            error:badarg ->
