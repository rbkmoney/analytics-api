...
process_request('GetClaims', Req, Context) ->
    ...
    Claims = yapi_utils:unwrap(
        yapi_handler_utils:service_call_with([user_info, party_id, party_creation], Call, Context)
    ),
...
process_request('GetClaimByID', Req, Context) ->
    Call = {
        party_management,
        'GetClaim',
        [yapi_handler_utils:get_party_id(Context), genlib:to_int(maps:get('claimID', Req))]
    },
    case yapi_handler_utils:service_call_with([user_info, party_creation], Call, Context) of
        {ok, Claim} ->
...
process_request('CreateClaim', Req, Context) ->
    try
        ...
        Call = {party_management, 'CreateClaim', [yapi_handler_utils:get_party_id(Context), Changeset]},
        case yapi_handler_utils:service_call_with([user_info, party_creation], Call, Context) of
            {ok, Claim} ->
...
%         ]},
%     Party = yapi_utils:unwrap(
%         yapi_handler_utils:service_call_with([user_info, party_id, party_creation], Call, Context)
%     ),
%     {ok, {200, #{}, yapi_handler_utils:yapi_handler_decoder_party:decode_party(Party)}};
    ...
    EncodedMod = case maps:get(<<"shopModificationType">>, Modification) of
        ...
        <<"ShopAccountCreation">> ->
            {shop_account_creation, #payproc_ShopAccountParams{
                currency = yapi_handler_encoder:encode_currency(maps:get(<<"currency">>, Modification))
            }};
        <<"ShopCategoryChange">> ->
...
encode_payout_tool_params(#{<<"currency">> := Currency, <<"details">> := Details}) ->
    #payproc_PayoutToolParams{
        currency = yapi_handler_encoder:encode_currency(Currency),
...
encode_international_bank_details(Acc) ->
    #domain_InternationalBankDetails{
        ...
        country = yapi_handler_encoder:encode_residence(genlib_map:get(<<"countryCode">>, Acc)),
...
encode_private_entity(#{<<"entityType">> := <<"RussianPrivateEntity">>} = Entity) ->
    {russian_private_entity, #domain_RussianPrivateEntity{
        ...
        contact_info = yapi_handler_encoder:encode_contact_info(maps:get(<<"contactInfo">>, Entity))
    }}.
...
decode_contract_modification({creation, ContractParams}) ->
    #{
        ...
        <<"contractor"              >> =>
            yapi_handler_decoder_party:decode_contractor(ContractParams#payproc_ContractParams.contractor),
        <<"paymentInstitutionID"    >> =>
            yapi_handler_decoder_party:decode_payment_institution_ref(
...
decode_contract_modification({legal_agreement_binding, LegalAgreement}) ->
    #{
        ...
        <<"legalAgreement"          >> => yapi_handler_decoder_party:decode_legal_agreement(LegalAgreement)
    };
...
decode_contract_modification({payout_tool_modification, #payproc_PayoutToolModificationUnit{
    ...
    #{
        ...
        <<"payoutToolID"            >> => PayoutToolID,
        <<"details"                 >> => yapi_handler_decoder_party:decode_payout_tool_details(ToolInfo)
    };
decode_contract_modification({report_preferences_modification, ReportPreferences}) ->
    maps:merge(
        ...
        yapi_handler_decoder_party:decode_reporting_preferences(ReportPreferences)
    ).
...
decode_shop_modification({payout_schedule_modification, #payproc_ScheduleModification{schedule = ScheduleRef}}) ->
    genlib_map:compact(#{
        ...
        <<"scheduleID"          >> => yapi_handler_decoder_utils:decode_business_schedule_ref(ScheduleRef)
    }).

decode_payout_tool_params(#payproc_PayoutToolParams{currency = Currency, tool_info = ToolInfo}) ->
    yapi_handler_decoder_party:decode_payout_tool_params(Currency, ToolInfo).

decode_shop_params(ShopParams) ->
    #{
        <<"location"    >> => yapi_handler_decoder_party:decode_shop_location(ShopParams#payproc_ShopParams.location),
        <<"details"     >> => yapi_handler_decoder_party:decode_shop_details(ShopParams#payproc_ShopParams.details),
