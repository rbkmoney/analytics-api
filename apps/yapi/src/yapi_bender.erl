...
get_idempotent_key(Prefix, PartyID, ExternalID) ->
    <<"yapi/", Prefix/binary, "/", PartyID/binary, "/", ExternalID/binary>>.
...
generate_id([Key, BenderSchema, Hash], WoodyCtx) ->
    Context = yapi_msgp_marshalling:marshal(#{
        <<"version">>     => ?SCHEMA_VER1,
        <<"params_hash">> => Hash
    }),
    Args = [Key, BenderSchema, Context],
    Result = case yapi_woody_client:call_service(bender, 'GenerateID', Args, WoodyCtx) of
        ...
        {ok, #bender_GenerationResult{internal_id = InternalID, context = Ctx}}       ->
            #{<<"params_hash">> := BenderHash} = yapi_msgp_marshalling:unmarshal(Ctx),
