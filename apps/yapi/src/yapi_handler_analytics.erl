...
create_stat_dsl(StatType, Req, Context) ->
    FromTime = yapi_handler_utils:get_time('fromTime', Req),
    ToTime   = yapi_handler_utils:get_time('toTime'  , Req),
    SplitInterval =
        case StatType of
            customers_rate_stat ->
                yapi_handler_utils:get_time_diff(FromTime, ToTime);
            _ ->
                SplitUnit = genlib_map:get('splitUnit', Req),
                SplitSize = genlib_map:get('splitSize', Req),
                yapi_handler_utils:get_split_interval(SplitSize, SplitUnit)
        end,
    Query = #{
    ...
    yapi_handler_utils:create_dsl(StatType, Query, #{}).

process_merchant_stat(StatType, Req, Context) ->
    Call = {
        merchant_stat,
        'GetStatistics',
        [yapi_handler_encoder:encode_stat_request(create_stat_dsl(StatType, Req, Context))]
    },
    process_merchant_stat_result(StatType, yapi_handler_utils:service_call(Call, Context)).
