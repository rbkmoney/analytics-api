-module(anapi_deadline_tests_SUITE).
...
init_per_group(deadline_header, Config) ->
    ...
    {ok, Token2} = anapi_ct_helper:issue_token(BasePermissions, unlimited),
    Context = anapi_ct_helper:get_context(Token),
...
deadline_error_test(_Config) ->
    ...
    {error, {400, _}} = anapi_client_categories:get_categories(get_context(Token, <<"blabla">>)).
...
deadline_absolute_ok_test(Config) ->
    ...
    ?badresp(504) = anapi_client_parties:get_my_party(Context#{deadline => BinDeadline}),
    Deadline2 = woody_deadline:from_timeout(3000),
    BinDeadline2 = woody_deadline:to_binary(Deadline2),
    {ok, _} = anapi_client_categories:get_categories(Context#{deadline => BinDeadline2}).
...
deadline_relative_ok_test(Config) ->
    ...
    ?badresp(504) = anapi_client_parties:get_my_party(Context),
    {ok, _} = anapi_client_categories:get_categories(Context).

get_context(Token, Deadline) ->
    DefEvtHandler = anapi_client_lib:default_event_handler(),
    anapi_client_lib:get_context(?CAPI_URL, Token, 10000, ipv4, #{}, DefEvtHandler, Deadline).
