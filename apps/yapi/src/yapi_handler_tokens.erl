...
-define(CAPI_NS, <<"com.rbkmoney.yapi">>).
...
process_request('CreatePaymentResource' = OperationID, Req, Context) ->
    ...
    try
        ...
        IdempotentKey = yapi_bender:get_idempotent_key(OperationID, PartyID, ExternalID),
        ...
        {ok, {201, #{}, yapi_handler_decoder_party:decode_disposable_payment_resource(PaymentResource)}}
    catch
        Result -> Result
    end;
...
is_ip_replacement_allowed(Context) ->
    Claims = yapi_handler_utils:get_auth_context(Context),
    case yapi_auth:get_claim(<<"ip_replacement_allowed">>, Claims, undefined) of
        <<"true">> ->
            true;
        _ ->
            false
...
put_session_to_cds(SessionID, SessionData, Context) ->
    ...
    {ok, ok} = yapi_handler_utils:service_call(Call, Context),
    ok.
...
put_card_data_to_cds(CardData, SessionData, {ExternalID, IdempotentKey}, Context) ->
    ...
    case yapi_bender:gen_by_constant(IdempotentKey, RandomID, Hash, WoodyCtx) of
        {ok, SessionID} ->
...
expand_card_info(BankCard, {BinData, Version}, HaveCVV) ->
    try
        BankCard#'domain_BankCard'{
            ...
            issuer_country = yapi_handler_encoder:encode_residence(BinData#'binbase_BinData'.iso_country_code),
...
process_tokenized_card_data(Data, IdempotentParams, Context) ->
    ...
    UnwrappedPaymentTool = case yapi_handler_utils:service_call(Call, Context) of
        {ok, Tool} ->
            Tool;
...
process_put_card_data_result(
...
) ->
    {
        {bank_card, BankCard#domain_BankCard{
            payment_system = PaymentSystem,
            masked_pan     = yapi_utils:define(Last4, BankCard#domain_BankCard.masked_pan),
...
encode_payment_request(#{<<"provider" >> := <<"ApplePay">>} = Data) ->
    {apple, #paytoolprv_ApplePayRequest{
        ...
        payment_token = yapi_handler_encoder:encode_content(json, maps:get(<<"paymentToken">>, Data))
    }};
