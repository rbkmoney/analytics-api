...
process_request('CreateInvoice' = OperationID, Req, Context) ->
    ...
    ExtraProperties = yapi_handler_utils:get_extra_properties(Context),
    InvoiceParams = maps:get('InvoiceParams', Req),
    try create_invoice(PartyID, InvoiceParams, Context, OperationID) of
        {ok, #'payproc_Invoice'{invoice = Invoice}} ->
            {ok, {201, #{}, yapi_handler_decoder_invoicing:make_invoice_and_token(Invoice, PartyID, ExtraProperties)}};
        {exception, Exception} ->
            case Exception of
...
process_request('CreateInvoiceAccessToken', Req, Context) ->
    ...
    case yapi_handler_utils:get_invoice_by_id(InvoiceID, Context) of
        {ok, #'payproc_Invoice'{}} ->
            Response =  yapi_handler_utils:issue_access_token(
                yapi_handler_utils:get_party_id(Context),
                {invoice, InvoiceID}
            ),
            {ok, {201, #{}, Response}};
        {exception, Exception} ->
...
process_request('GetInvoiceByID', Req, Context) ->
    case yapi_handler_utils:get_invoice_by_id(maps:get(invoiceID, Req), Context) of
        {ok, #'payproc_Invoice'{invoice = Invoice}} ->
            {ok, {200, #{}, yapi_handler_decoder_invoicing:decode_invoice(Invoice)}};
        {exception, Exception} ->
            case Exception of
                #payproc_InvalidUser{} ->
                    {ok, general_error(404, <<"Invoice not found">>)};
...
process_request('FulfillInvoice', Req, Context) ->
    ...
    case yapi_handler_utils:service_call_with([user_info], Call, Context) of
        {ok, _} ->
            {ok, {204, #{}, undefined}};
        {exception, Exception} ->
            case Exception of
...
process_request('GetInvoiceEvents', Req, Context) ->
    Result =
        yapi_handler_utils:collect_events(
            maps:get(limit, Req),
            genlib_map:get(eventID, Req),
            fun(Range) ->
                yapi_handler_utils:service_call_with(
                    [user_info],
...
process_request('GetInvoicePaymentMethods', Req, Context) ->
    case yapi_handler_decoder_invoicing:construct_payment_methods(invoicing, [maps:get(invoiceID, Req)], Context) of
...
create_invoice(PartyID, InvoiceParams, #{woody_context := WoodyCtx} = Context, BenderPrefix) ->
    ...
    IdempotentKey = yapi_bender:get_idempotent_key(BenderPrefix, PartyID, ExternalID),
    Hash = erlang:phash2(InvoiceParams),
    case yapi_bender:gen_by_snowflake(IdempotentKey, Hash, WoodyCtx) of
        {ok, ID} ->
            Call = {invoicing, 'Create', [encode_invoice_params(ID, PartyID, InvoiceParams)]},
            yapi_handler_utils:service_call_with([user_info, party_creation], Call, Context);
...
encode_invoice_params(ID, PartyID, InvoiceParams) ->
    ...
    #payproc_InvoiceParams{
        ...
        due      = yapi_handler_utils:get_time(<<"dueDate">>, InvoiceParams),
        context  = yapi_handler_encoder:encode_invoice_context(InvoiceParams),
...
encode_invoice_cost(Amount, Currency, Cart) when Amount =/= undefined, Cart =/= undefined ->
    case get_invoice_cart_amount(Cart) of
        Amount ->
            yapi_handler_encoder:encode_cash(Amount, Currency);
        _ ->
            throw(invalid_invoice_cost)
    end;
encode_invoice_cost(undefined, Currency, Cart) when Cart =/= undefined ->
    yapi_handler_encoder:encode_cash(get_invoice_cart_amount(Cart), Currency);
...
encode_invoice_details(Params) ->
    #domain_InvoiceDetails{
        ...
        cart        = yapi_handler_encoder:encode_invoice_cart(Params)
    }.
...
decode_invoice_change(_, {invoice_created, #payproc_InvoiceCreated{invoice = Invoice}}, _Context) ->
    #{
        ...
        <<"invoice"   >> => yapi_handler_decoder_invoicing:decode_invoice(Invoice)
    };
...
decode_payment_change(InvoiceID, _PaymentID, {invoice_payment_started, PaymentStarted}, Context) ->
    ...
    #{
        ...
        <<"payment"   >> => yapi_handler_decoder_invoicing:decode_payment(InvoiceID, Payment, Context)
    };
...
decode_payment_change(_InvoiceID, PaymentID, {invoice_payment_session_change,
    ...
    #{
        ...
        <<"paymentID">> => PaymentID,
        <<"userInteraction">> => yapi_handler_decoder_invoicing:decode_user_interaction(Interaction)
    };
...
decode_payment_change(_InvoiceID, PaymentID, {invoice_payment_status_changed, PaymentStatusChanged}, Context) ->
    ...
    yapi_handler_utils:merge_and_compact(
        #{
            <<"changeType">> => <<"PaymentStatusChanged">>,
            <<"paymentID" >> => PaymentID
        },
        yapi_handler_decoder_invoicing:decode_payment_status(Status, Context)
    );
...
decode_refund_change(_, PaymentID, RefundID, {invoice_payment_refund_status_changed, StatusChanged}, Context) ->
    ...
    yapi_handler_utils:merge_and_compact(
        ...
        yapi_handler_decoder_invoicing:decode_refund_status(Status, Context)
    );
...
decode_refund_for_event(#domain_InvoicePaymentRefund{cash = #domain_Cash{}} = Refund, _, _, Context) ->
    yapi_handler_decoder_invoicing:decode_refund(Refund, Context);
decode_refund_for_event(#domain_InvoicePaymentRefund{cash = undefined} = Refund, InvoiceID, PaymentID, Context) ->
    ...
    {ok, #payproc_InvoicePayment{payment = #domain_InvoicePayment{cost = Cash}}} =
        yapi_handler_utils:get_payment_by_id(InvoiceID, PaymentID, Context),
    yapi_handler_decoder_invoicing:decode_refund(Refund#domain_InvoicePaymentRefund{cash = Cash}, Context).
