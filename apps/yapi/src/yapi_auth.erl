...
-type context () :: yapi_authorizer_jwt:t().
-type claims  () :: yapi_authorizer_jwt:claims().
...
authorize_api_key(_OperationID, bearer, Token) ->
    ...
    yapi_authorizer_jwt:verify(Token).
...
authorize_operation(OperationID, Req, {{_SubjectID, ACL}, _}) ->
    ...
    case lists:all(
        fun ({Scope, Permission}) ->
            lists:member(Permission, yapi_acl:match(Scope, ACL))
        end,
        Access
    ) of
        true ->
...
issue_access_token(PartyID, TokenSpec, ExtraProperties) ->
    ...
    yapi_utils:unwrap(yapi_authorizer_jwt:issue({{PartyID, yapi_acl:from_list(ACL)}, Claims}, Expiration)).

-type acl() :: [{yapi_acl:scope(), yapi_acl:permission()}].

-spec resolve_token_spec(token_spec()) ->
    {claims(), acl(), yapi_authorizer_jwt:expiration()}.
...
-spec get_operation_access(swag_server:operation_id(), request_data()) ->
    [{yapi_acl:scope(), yapi_acl:permission()}].
