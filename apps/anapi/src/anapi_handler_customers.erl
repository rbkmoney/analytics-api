-module(anapi_handler_customers).
...
process_request('CreateCustomer', Req, Context) ->
    ...
    case anapi_handler_utils:service_call_with([party_creation], Call, Context) of
        {ok, Customer} ->
...
process_request('DeleteCustomer', Req, Context) ->
    case anapi_handler_utils:service_call({customer_management, 'Delete', [maps:get(customerID, Req)]}, Context) of
        {ok, _} ->
            {ok, {204, #{}, undefined}};
        {exception, Exception} ->
            case Exception of
...
process_request('CreateBinding', Req, Context) ->
    ...
    Result =
        try
            anapi_handler_utils:service_call({customer_management, 'StartBinding', CallArgs}, Context)
        catch
...
process_request('GetCustomerEvents', Req, Context) ->
    GetterFun =
        fun(Range) ->
            anapi_handler_utils:service_call(
...
get_customer_by_id(CustomerID, Context) ->
    anapi_handler_utils:service_call({customer_management, 'Get', [CustomerID]}, Context).

encode_customer_params(PartyID, Params) ->
    #payproc_CustomerParams{
        party_id     = PartyID,
        shop_id      = genlib_map:get(<<"shopID">>, Params),
        contact_info = anapi_handler_encoder:encode_contact_info(genlib_map:get(<<"contactInfo">>, Params)),
        metadata     = encode_customer_metadata(genlib_map:get(<<"metadata">>, Params))
    }.

encode_customer_metadata(Meta) ->
    anapi_json_marshalling:marshal(Meta).


encode_customer_binding_params(#{<<"paymentResource">> := PaymentResource}) ->
    PaymentTool = anapi_handler_encoder:encode_payment_tool_token(maps:get(<<"paymentToolToken">>, PaymentResource)),
    {ClientInfo, PaymentSession} =
        anapi_handler_utils:unwrap_payment_session(maps:get(<<"paymentSession">>, PaymentResource)),
...
make_customer_and_token(Customer, PartyID) ->
    #{
        ...
        <<"customerAccessToken">> =>
            anapi_handler_utils:issue_access_token(PartyID, {customer, Customer#payproc_Customer.id})
    }.
...
decode_customer(Customer) ->
    #{
        ...
        <<"contactInfo">> =>
            anapi_handler_decoder_party:decode_contact_info(Customer#payproc_Customer.contact_info),
...
decode_customer_metadata(Meta) ->
    anapi_json_marshalling:unmarshal(Meta).
...
decode_customer_binding(CustomerBinding, Context) ->
    anapi_handler_utils:merge_and_compact(
        #{
            ...
            <<"paymentResource">> =>
                anapi_handler_decoder_party:decode_disposable_payment_resource(
...
decode_customer_binding_change(BindingID, {status_changed, StatusChange}, Context) ->
    ...
    {true, anapi_handler_utils:merge_and_compact(
        #{
...
decode_customer_binding_change(BindingID, {interaction_requested, InteractionRequest}, _) ->
    ...
    {true, #{
        ...
        <<"customerBindingID">> => BindingID,
        <<"userInteraction">> => anapi_handler_decoder_invoicing:decode_user_interaction(UserInteraction)
    }}.
