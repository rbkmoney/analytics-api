...
process_request('CreateWebhook', Req, Context) ->
    WebhookParams = encode_webhook_params(anapi_handler_utils:get_party_id(Context), maps:get('Webhook', Req)),
    ...
    case anapi_handler_utils:service_call_with([user_info, party_id, party_creation], Call, Context) of
        {ok, _} ->
            Webhook = anapi_utils:unwrap(
                anapi_handler_utils:service_call({webhook_manager, 'Create', [WebhookParams]}, Context)
            ),
...
process_request('GetWebhooks', _Req, Context) ->
    Webhooks = anapi_utils:unwrap(
        anapi_handler_utils:service_call_with([party_id], {webhook_manager, 'GetList', []}, Context)
    ),
...
get_webhook(WebhookID, Context) ->
    ...
    case anapi_handler_utils:service_call({webhook_manager, 'Get', [WebhookID]}, Context) of
...
delete_webhook(WebhookID, Context) ->
    case get_webhook(WebhookID, Context) of
        {ok, #webhooker_Webhook{}} ->
            anapi_handler_utils:service_call({webhook_manager, 'Delete', [WebhookID]}, Context);
        Exception ->
            Exception
    end.
