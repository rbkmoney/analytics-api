...
create_invoice_ok_test(Config) ->
    anapi_ct_helper:mock_services([
        {bender,  fun('GenerateID', _) -> {ok, anapi_ct_helper_bender:get_result(<<"bender key">>)} end},
    ...
    {ok, _} = anapi_client_invoices:create_invoice(?config(context, Config), Req).
...
create_invoice_idemp_ok_test(Config) ->
    ...
    anapi_ct_helper:mock_services([
        ...
        {bender,    fun('GenerateID', _) -> {ok, anapi_ct_helper_bender:get_result(BenderKey)} end}
    ], Config),
    Req = #{
        <<"shopID">>      => ?STRING,
        <<"amount">>      => ?INTEGER,
    ...
    {ok, #{<<"invoice">> := Invoice}}  = anapi_client_invoices:create_invoice(?config(context, Config), Req),
    {ok, #{<<"invoice">> := Invoice2}} = anapi_client_invoices:create_invoice(?config(context, Config), Req),
...
create_invoice_idemp_fail_test(Config) ->
    ...
    Ctx = anapi_msgp_marshalling:marshal(#{<<"params_hash">> => erlang:phash2(Req)}),
    anapi_ct_helper:mock_services([
        {invoicing, fun('Create', [_UserInfo, #payproc_InvoiceParams{id = ID, external_id = EID}]) ->
            {ok, ?PAYPROC_INVOICE_WITH_ID(ID, EID)}
        end},
        {bender, fun('GenerateID', _) -> {ok, anapi_ct_helper_bender:get_result(BenderKey, Ctx)} end}
    ], Config),
    ...
    Response = anapi_client_invoices:create_invoice(
...
create_invoice_access_token_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('Get', _) -> {ok, ?PAYPROC_INVOICE} end}], Config),
    {ok, _} = anapi_client_invoices:create_invoice_access_token(?config(context, Config), ?STRING).

-spec create_invoice_template_ok_test(config()) ->
    _.
create_invoice_template_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoice_templating, fun('Create', _) -> {ok, ?INVOICE_TPL} end}], Config),
    Req = #{
        <<"shopID">> => ?STRING,
        <<"lifetime">> => anapi_ct_helper:get_lifetime(),
    ...
    {ok, _} = anapi_client_invoice_templates:create(?config(context, Config), Req#{<<"details">> => Details0}),
    Details1 = #{
    ...
    {ok, _} = anapi_client_invoice_templates:create(?config(context, Config), Req#{<<"details">> => Details1}).
...
create_invoice_with_template_test(Config) ->
    ...
    {ok, Template} =
        anapi_client_invoice_templates:create(?config(context, Config), ReqTemp#{<<"details">> => Details}),
    ...
    {ok, #{<<"invoice">> := Invoice}} =
        anapi_client_invoice_templates:create_invoice(?config(context, Config), TemplateID, Req),
    ?assertEqual(BenderKey,  maps:get(<<"id">>, Invoice)),
    ?assertEqual(ExternalID, maps:get(<<"externalID">>, Invoice)).
...
create_customer_ok_test(Config) ->
    anapi_ct_helper:mock_services([{customer_management, fun('Create', _) -> {ok, ?CUSTOMER} end}], Config),
    Req = #{
        <<"shopID">> => ?STRING,
        <<"contactInfo">> => #{<<"email">> => <<"bla@bla.ru">>},
        <<"metadata">> => #{<<"text">> => [<<"SOMESHIT">>, 42]}
    },
    {ok, _} = anapi_client_customers:create_customer(?config(context, Config), Req).

-spec create_customer_access_token_ok_test(config()) ->
    _.
create_customer_access_token_ok_test(Config) ->
    anapi_ct_helper:mock_services([{customer_management, fun('Get', _) -> {ok, ?CUSTOMER} end}], Config),
    {ok, _} = anapi_client_customers:create_customer_access_token(?config(context, Config), ?STRING).

-spec delete_customer_ok_test(config()) ->
    _.
delete_customer_ok_test(Config) ->
    anapi_ct_helper:mock_services([{customer_management, fun('Delete', _) -> {ok, ok} end}], Config),
    {ok, _} = anapi_client_customers:delete_customer(?config(context, Config), ?STRING).

-spec rescind_invoice_ok_test(config()) ->
    _.
rescind_invoice_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('Rescind', _) -> {ok, ok} end}], Config),
    ok = anapi_client_invoices:rescind_invoice(?config(context, Config), ?STRING, ?STRING).

-spec fulfill_invoice_ok_test(config()) ->
    _.
fulfill_invoice_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('Fulfill', _) -> {ok, ok} end}], Config),
    ok = anapi_client_invoices:fulfill_invoice(?config(context, Config), ?STRING, ?STRING).
...
get_failed_payment_with_invalid_cvv(Config) ->
    ...
    anapi_client_payments:get_payment_by_id(?config(context, Config), ?STRING, ?STRING).


-spec create_refund(config()) ->
    _.
create_refund(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('RefundPayment', _) -> {ok, ?REFUND} end}], Config),
    Req = #{<<"reason">> => ?STRING},
    {ok, _} = anapi_client_payments:create_refund(?config(context, Config), Req, ?STRING, ?STRING).
...
create_refund_error(Config) ->
    ...
    Req = #{<<"reason">> => ?STRING},
    {error, {400, _}} = anapi_client_payments:create_refund(?config(context, Config), Req, <<"42">>, ?STRING),
    {error, {400, _}} = anapi_client_payments:create_refund(?config(context, Config), Req, <<"43">>, ?STRING).
...
get_refund_by_id(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('GetPaymentRefund', _) -> {ok, ?REFUND} end}], Config),
    {ok, _} = anapi_client_payments:get_refund_by_id(?config(context, Config), ?STRING, ?STRING, ?STRING).

-spec get_refunds(config()) ->
    _.
get_refunds(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('GetPayment', _) -> {ok, ?PAYPROC_PAYMENT} end}], Config),
    {ok, _} = anapi_client_payments:get_refunds(?config(context, Config), ?STRING, ?STRING).

-spec update_invoice_template_ok_test(config()) ->
    _.
update_invoice_template_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoice_templating, fun('Update', _) -> {ok, ?INVOICE_TPL} end}], Config),
    Req = #{
        <<"lifetime">> => anapi_ct_helper:get_lifetime(),
        <<"metadata">> => #{<<"invoice_template_dummy_metadata">> => <<"test_value">>}
    },
    ...
    {ok, _} = anapi_client_invoice_templates:update(?config(context, Config), ?STRING, Req#{<<"details">> => Details0}),
    Details1 = #{
        <<"templateType">> => <<"InvoiceTemplateMultiLine">>,
        <<"currency">> => ?RUB,
        <<"cart">> => [
    ...
    {ok, _} = anapi_client_invoice_templates:update(?config(context, Config), ?STRING, Req#{<<"details">> => Details1}).

-spec delete_invoice_template_ok_test(config()) ->
    _.
delete_invoice_template_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoice_templating, fun('Delete', _) -> {ok, ok} end}], Config),
    ok = anapi_client_invoice_templates:delete(?config(context, Config), ?STRING).

-spec get_account_by_id_ok_test(config()) ->
    _.
get_account_by_id_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('GetAccountState', _) -> {ok, ?ACCOUNT_STATE} end}], Config),
    {ok, _} = anapi_client_accounts:get_account_by_id(?config(context, Config), ?INTEGER).

-spec get_my_party_ok_test(config()) ->
    _.
get_my_party_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('Get', _) -> {ok, ?PARTY} end}], Config),
    {ok, _} = anapi_client_parties:get_my_party(?config(context, Config)).

-spec suspend_my_party_ok_test(config()) ->
    _.
suspend_my_party_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('Suspend', _) -> {ok, ok} end}], Config),
    ok = anapi_client_parties:suspend_my_party(?config(context, Config)).

-spec activate_my_party_ok_test(config()) ->
    _.
activate_my_party_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('Activate', _) -> {ok, ok} end}], Config),
    ok = anapi_client_parties:activate_my_party(?config(context, Config)).

-spec get_shop_by_id_ok_test(config()) ->
    _.
get_shop_by_id_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('GetShop', _) -> {ok, ?SHOP} end}], Config),
    {ok, _} = anapi_client_shops:get_shop_by_id(?config(context, Config), ?STRING).

-spec get_shops_ok_test(config()) ->
    _.
get_shops_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('Get', _) -> {ok, ?PARTY} end}], Config),
    {ok, _} = anapi_client_shops:get_shops(?config(context, Config)).

-spec activate_shop_ok_test(config()) ->
    _.
activate_shop_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('ActivateShop', _) -> {ok, ok} end}], Config),
    ok = anapi_client_shops:activate_shop(?config(context, Config), ?STRING).

-spec suspend_shop_ok_test(config()) ->
    _.
suspend_shop_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('SuspendShop', _) -> {ok, ok} end}], Config),
    ok = anapi_client_shops:suspend_shop(?config(context, Config), ?STRING).
...
get_claim_by_id_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_claims:get_claim_by_id(?config(context, Config), ?INTEGER).
...
get_claims_ok_test(Config) ->
    ...
    {ok, [_OnlyOneClaim]} = anapi_client_claims:get_claims(?config(context, Config)).

-spec revoke_claim_ok_test(config()) ->
    _.
revoke_claim_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('RevokeClaim', _) -> {ok, ok} end}], Config),
    ok = anapi_client_claims:revoke_claim_by_id(?config(context, Config), ?STRING, ?INTEGER, ?INTEGER).
...
create_claim_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_claims:create_claim(?config(context, Config), Changeset).
...
create_claim_invalid_residence_test(Config) ->
    ...
    {error, {400, _}} = anapi_client_claims:create_claim(?config(context, Config), Changeset).

-spec get_contract_by_id_ok_test(config()) ->
    _.
get_contract_by_id_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('Get', _) -> {ok, ?PARTY} end}], Config),
    {ok, _} = anapi_client_contracts:get_contract_by_id(?config(context, Config), ?STRING),
    {ok, _} = anapi_client_contracts:get_contract_by_id(?config(context, Config), ?WALLET_CONTRACT_ID).

-spec get_contracts_ok_test(config()) ->
    _.
get_contracts_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('Get', _) -> {ok, ?PARTY} end}], Config),
    {ok, [_First, _Second]} = anapi_client_contracts:get_contracts(?config(context, Config)).

-spec get_contract_adjustments_ok_test(config()) ->
    _.
get_contract_adjustments_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('GetContract', _) -> {ok, ?CONTRACT} end}], Config),
    {ok, _} = anapi_client_contracts:get_contract_adjustments(?config(context, Config), ?STRING).

-spec get_contract_adjustment_by_id_ok_test(config()) ->
    _.
get_contract_adjustment_by_id_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('GetContract', _) -> {ok, ?CONTRACT} end}], Config),
    {ok, _} = anapi_client_contracts:get_contract_adjustment_by_id(?config(context, Config), ?STRING, ?STRING).

-spec get_payout_tools_ok_test(config()) ->
    _.
get_payout_tools_ok_test(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('GetContract', _) -> {ok, ?CONTRACT} end}], Config),
    {ok, _} = anapi_client_payouts:get_payout_tools(?config(context, Config), ?STRING).

-spec get_payout_tool_by_id(config()) ->
    _.
get_payout_tool_by_id(Config) ->
    anapi_ct_helper:mock_services([{party_management, fun('GetContract', _) -> {ok, ?CONTRACT} end}], Config),
    {ok, _} = anapi_client_payouts:get_payout_tool_by_id(?config(context, Config), ?STRING, ?BANKID_RU),
    {ok, _} = anapi_client_payouts:get_payout_tool_by_id(?config(context, Config), ?STRING, ?BANKID_US),
    {ok, _} = anapi_client_payouts:get_payout_tool_by_id(?config(context, Config), ?STRING, ?WALLET_TOOL).

-spec create_payout(config()) ->
    _.
create_payout(Config) ->
    Payout = ?PAYOUT(?WALLET_PAYOUT_TYPE, []),
    anapi_ct_helper:mock_services([{payouts, fun('CreatePayout', _) -> {ok, Payout} end}], Config),
    Req = #{
        <<"id">> => ?STRING,
        <<"shopID">> => ?STRING,
    ...
    {ok, _} = anapi_client_payouts:create_payout(?config(context, Config), Req, ?STRING).

-spec get_payout(config()) ->
    _.
get_payout(Config) ->
    Payout = ?PAYOUT(?WALLET_PAYOUT_TYPE, [?PAYOUT_PROC_PAYOUT_SUMMARY_ITEM]),
    anapi_ct_helper:mock_services([{payouts, fun('Get', _) -> {ok, Payout} end}], Config),
    {ok, _} = anapi_client_payouts:get_payout(?config(context, Config), ?STRING).
...
get_payout_fail(Config) ->
    Payout = ?PAYOUT(?WALLET_PAYOUT_TYPE, [?PAYOUT_PROC_PAYOUT_SUMMARY_ITEM]),
    anapi_ct_helper:mock_services([{payouts, fun('Get', _) -> {ok, Payout} end}], Config),
    {error, {404, _}} = anapi_client_payouts:get_payout(?config(context_with_diff_party, Config), ?STRING).
...
create_webhook_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_webhooks:create_webhook(?config(context, Config), Req).

-spec get_webhooks(config()) ->
    _.
get_webhooks(Config) ->
    anapi_ct_helper:mock_services([{webhook_manager, fun('GetList', _) -> {ok, [?WEBHOOK]} end}], Config),
    {ok, _} = anapi_client_webhooks:get_webhooks(?config(context, Config)).

-spec get_webhook_by_id(config()) ->
    _.
get_webhook_by_id(Config) ->
    anapi_ct_helper:mock_services([{webhook_manager, fun('Get', _) -> {ok, ?WEBHOOK} end}], Config),
    {ok, _} = anapi_client_webhooks:get_webhook_by_id(?config(context, Config), ?INTEGER_BINARY).
...
delete_webhook_by_id(Config) ->
    ...
    ok = anapi_client_webhooks:delete_webhook_by_id(?config(context, Config), ?INTEGER_BINARY).

-spec get_locations_names_ok_test(config()) ->
    _.
get_locations_names_ok_test(Config) ->
    anapi_ct_helper:mock_services([{geo_ip_service, fun('GetLocationName', _) -> {ok, #{123 => ?STRING}} end}], Config),
    Query = #{
        <<"geoIDs">> => <<"5,3,6,5,4">>,
        <<"language">> => <<"ru">>
    },
    {ok, _} = anapi_client_geo:get_location_names(?config(context, Config), Query).
...
get_payment_conversion_stats_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_analytics:get_payment_conversion_stats(?config(context, Config), Query).
...
get_payment_revenue_stats_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_analytics:get_payment_revenue_stats(?config(context, Config), Query).
...
get_payment_geo_stats_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_analytics:get_payment_geo_stats(?config(context, Config), Query).
...
get_payment_rate_stats_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_analytics:get_payment_rate_stats(?config(context, Config), Query).
...
get_payment_method_stats_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_analytics:get_payment_method_stats(?config(context, Config), Query).
...
get_categories_ok_test(Config) ->
    {ok, _} = anapi_client_categories:get_categories(?config(context, Config)).

-spec get_category_by_ref_ok_test(config()) ->
    _.
get_category_by_ref_ok_test(Config) ->
    {ok, _} = anapi_client_categories:get_category_by_ref(?config(context, Config), ?INTEGER).

-spec get_schedule_by_ref_ok_test(config()) ->
    _.
get_schedule_by_ref_ok_test(Config) ->
    {ok, _} = anapi_client_payouts:get_schedule_by_ref(?config(context, Config), ?INTEGER).

-spec get_payment_institutions(config()) ->
    _.
get_payment_institutions(Config) ->
    {ok, [_Something]} = anapi_client_payment_institutions:get_payment_institutions(?config(context, Config)),
    {ok, []} =
        anapi_client_payment_institutions:get_payment_institutions(?config(context, Config), <<"RUS">>, <<"live">>),
    {ok, [#{<<"realm">> := <<"test">>}]} =
        anapi_client_payment_institutions:get_payment_institutions(?config(context, Config), <<"RUS">>, <<"test">>).

-spec get_payment_institution_by_ref(config()) ->
    _.
get_payment_institution_by_ref(Config) ->
    {ok, _} = anapi_client_payment_institutions:get_payment_institution_by_ref(?config(context, Config), ?INTEGER).
...
get_payment_institution_payment_terms(Config) ->
    ...
    {ok, _} = anapi_client_payment_institutions:get_payment_institution_payment_terms(
        ?config(context, Config),
        ?INTEGER
    ).
...
get_payment_institution_payout_terms(Config) ->
    ...
    {ok, _} = anapi_client_payment_institutions:get_payment_institution_payout_methods(
        ?config(context, Config),
        ?INTEGER,
        <<"RUB">>
    ),
    {ok, _} = anapi_client_payment_institutions:get_payment_institution_payout_schedules(
        ?config(context, Config),
        ?INTEGER,
        <<"USD">>,
        <<"BankAccount">>
