-module(anapi_invoice_access_token_tests_SUITE).
...
init_per_group(operations_by_invoice_access_token_after_invoice_creation, Config) ->
    MockServiceSup = anapi_ct_helper:start_mocked_service_sup(?MODULE),
    ExtraProperties = #{<<"ip_replacement_allowed">> => <<"true">>},
    {ok, Token} = anapi_ct_helper:issue_token([{[invoices], write}], unlimited, ExtraProperties),
    anapi_ct_helper:mock_services([
        {invoicing, fun('Create', _) -> {ok, ?PAYPROC_INVOICE} end},
        {bender,    fun('GenerateID', _) -> {ok, anapi_ct_helper_bender:get_result(<<"bender_key">>)} end}
    ], MockServiceSup),
    Req = #{
        <<"shopID">> => ?STRING,
        <<"amount">> => ?INTEGER,
    ...
    } = anapi_client_invoices:create_invoice(anapi_ct_helper:get_context(Token, ExtraProperties), Req),
    anapi_ct_helper:stop_mocked_service_sup(MockServiceSup),
    [{context, anapi_ct_helper:get_context(InvAccToken)} | Config];

init_per_group(operations_by_invoice_access_token_after_token_creation, Config) ->
    MockServiceSup = anapi_ct_helper:start_mocked_service_sup(?MODULE),
    {ok, Token} = anapi_ct_helper:issue_token([{[invoices], write}], unlimited),
    anapi_ct_helper:mock_services([
        {invoicing, fun('Get', _) -> {ok, ?PAYPROC_INVOICE} end},
        {bender,    fun('GenerateID', _) -> {ok, anapi_ct_helper_bender:get_result(<<"bender_key">>)} end}
    ], MockServiceSup),
    {ok, #{<<"payload">> := InvAccToken}
    } = anapi_client_invoices:create_invoice_access_token(anapi_ct_helper:get_context(Token), ?STRING),
    anapi_ct_helper:stop_mocked_service_sup(MockServiceSup),
    [{context, anapi_ct_helper:get_context(InvAccToken)} | Config];
...
create_visa_payment_resource_ok_test(Config) ->
    ...
    }}} = anapi_client_tokens:create_payment_resource(?config(context, Config), #{
        <<"paymentTool">> => #{
...
create_visa_payment_resource_idemp_ok_test(Config) ->
    ...
    }} = anapi_client_tokens:create_payment_resource(?config(context, Config), Params),
    {ok, #{
        <<"paymentToolToken">> := ToolToken,
        <<"paymentSession">>   := ToolSession,
        <<"paymentToolDetails">> := PaymentToolDetails
...
create_visa_payment_resource_idemp_fail_test(Config) ->
    ...
    Token1 = <<"TOKEN1">>,
    Token2 = <<"TOKEN2">>,
    Ctx = anapi_msgp_marshalling:marshal(#{<<"params_hash">> => erlang:phash2(Token1)}),
    anapi_ct_helper:mock_services([
        {cds_storage, fun
            ('PutSession', _) -> {ok, ok};
            ('PutCard', [
    ...
    {ok, _} = anapi_client_tokens:create_payment_resource(?config(context, Config), Params),
    {error, {409, #{
        <<"externalID">> := ExternalID,
        <<"message">>    := <<"This 'externalID' has been used by another request">>
    }}} = anapi_client_tokens:create_payment_resource(?config(context, Config), Params2).
...
ip_replacement_not_allowed_test(Config) ->
    ...
    {ok, Res} = anapi_client_tokens:create_payment_resource(?config(context, Config), #{
        <<"paymentTool">> => #{
            <<"paymentToolType">> => <<"DigitalWalletData">>,
            <<"digitalWalletType">> => <<"DigitalWalletQIWI">>,
            <<"phoneNumber">> => <<"+79876543210">>
...
get_invoice_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_invoices:get_invoice_by_id(?config(context, Config), ?STRING).
...
get_invoice_events_ok_test(Config) ->
    ...
    {ok, [#{<<"id">> := 1}, #{<<"id">> := 2}, #{<<"id">> := 4}]} =
        anapi_client_invoices:get_invoice_events(?config(context, Config), ?STRING, 3),
    {ok, [#{<<"id">> := 4}, #{<<"id">> := 7}]} =
        anapi_client_invoices:get_invoice_events(?config(context, Config), ?STRING, 2, 3).

-spec get_invoice_payment_methods_ok_test(config()) ->
    _.
get_invoice_payment_methods_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('ComputeTerms', _) -> {ok, ?TERM_SET} end}], Config),
    {ok, _} = anapi_client_invoices:get_invoice_payment_methods(?config(context, Config), ?STRING).
...
create_payment_ok_test(Config) ->
    ...
    }} = anapi_client_tokens:create_payment_resource(?config(context, Config), Req1),
    Req2 = #{
        <<"externalID">> => ExternalID,
    ...
    {ok, #{<<"id">> := BenderKey, <<"externalID">> := ExternalID}} =
        anapi_client_payments:create_payment(?config(context, Config), Req2, ?STRING).
...
create_payment_with_empty_cvv_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_payments:create_payment(?config(context, Config), Req2, ?STRING).
...
get_payments_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('Get', _) -> {ok, ?PAYPROC_INVOICE} end}], Config),
    {ok, _} = anapi_client_payments:get_payments(?config(context, Config), ?STRING).
...
get_payment_by_id_ok_test(Config) ->
    ...
    {ok, _} = anapi_client_payments:get_payment_by_id(?config(context, Config), ?STRING, ?STRING).
...
cancel_payment_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('CancelPayment', _) -> {ok, ok} end}], Config),
    ok = anapi_client_payments:cancel_payment(?config(context, Config), ?STRING, ?STRING, ?STRING).

-spec capture_payment_ok_test(config()) ->
    _.
capture_payment_ok_test(Config) ->
    anapi_ct_helper:mock_services([{invoicing, fun('CapturePaymentNew', _) -> {ok, ok} end}], Config),
    Req = #{
        <<"reason">> => ?STRING
    },
    ok = anapi_client_payments:capture_payment(?config(context, Config), Req, ?STRING, ?STRING).
...
get_recurrent_payments_ok_test(Config) ->
    ...
    anapi_ct_helper:mock_services([{invoicing, fun('Get', _) -> {ok, Invoice} end}], Config),
    {ok, _} = anapi_client_payments:get_payments(?config(context, Config), ?STRING).
