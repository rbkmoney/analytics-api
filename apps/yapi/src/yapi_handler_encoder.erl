...
-type request_data() :: yapi_handler:request_data().
...
encode_payment_tool_token(Token) ->
    try yapi_utils:base64url_to_map(Token) of
...
encode_bank_card_metadata(Meta) ->
    maps:map(fun(_, Data) -> yapi_msgp_marshalling:marshal(Data) end, Meta).
